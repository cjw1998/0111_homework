...
void CenterFrame::createUserCommandArea()
{
    ...
    connect(btnRect,&QPushButton::clicked,this,&CenterFrame::on_btnRectClicked);
    ...
    connect(btnEllipse,&QPushButton::clicked,this,&CenterFrame::on_btnEllipseClicked);
    ...
    connect(btnLine,&QPushButton::clicked,this,&CenterFrame::on_btnLineClicked);
    ...
    p.fill(BACKGROUND_COLOR);
    // 三角形的三个顶点
    QPointF pt1(3,p.size().height()-3);//zuoxia
    QPointF pt2(p.size().width()/2,3);//zhongjian
    QPointF pt3(-3+p.size().width(),-3+p.size().height());//youxia
    QVector<QPointF> pts;
    ...
    connect(btnText,&QPushButton::clicked,this,&CenterFrame::on_btnTextClicked);
    //绘制菱形
    ...
    p.fill(BACKGROUND_COLOR);
    QPointF npt1(p.size().width()/2,p.size().height()-3);
    QPointF npt2(3,p.size().height()/2);
    QPointF npt3(p.size().width()/2,3);
    QPointF npt4(-3+p.size().width(),p.size().height()/2);
    QVector<QPointF> npts;
    npts<<npt1<<npt2<<npt2<<npt3<<npt3<<npt4<<npt4<<npt1;
    //npts<<npt1<<npt2;
    painter.drawLines(npts);
    //painter.drawEllipse(3,3,p.size().width()-2*3,p.size().height()-2*3);
    btnDiamond->setIcon (QIcon(p));
    connect(btnDiamond,&QPushButton::clicked,this,&CenterFrame::on_btnDiamondClicked);

    //绘图按钮
    btnimage = new QPushButton(group);
    btnimage->setToolTip("绘制图片");
    ...
    connect(btnimage,&QPushButton::clicked,this,&CenterFrame::on_btnImgClicked);
    ...
    gridLayout->addWidget(btnimage,3,0);
