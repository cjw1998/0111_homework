...
canDraw = true;
 ...
 void DrawWidget::drawImg()
{
    QImage image;
    QString filename=QFileDialog::getOpenFileName(this,tr("选择图像"),"",tr("Images (*.png *.bmp *.jpg)"));
    image.load(filename);    //读取图片
    QPixmap *newPix = new QPixmap(size());
    *newPix=QPixmap(*this->pix);
    *pix = QPixmap::fromImage(image.scaledToWidth(pix->size().width()*0.5, Qt::FastTransformation));
    QPainter p(newPix);                             //在画板中间画图
